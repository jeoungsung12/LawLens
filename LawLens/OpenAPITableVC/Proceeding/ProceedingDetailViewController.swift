//
//  ProceedingDetailViewController.swift
//  LawLens
//
...
class ProceedingDetailViewController: UIViewController {
    let post : ProceedingAPIModel
    var pln = UITextView()
    //이니셜라이저를 사용하여 Post 객체를 전달받아 post 속성에 저장
    init(post: ProceedingAPIModel) {
        self.post = post
    ...
    func setupViews() {
        ...
        hospitalBtn.setTitle("  관련기사 보기", for: .normal)
        ...
        hospitalBtn.addTarget(self, action: #selector(ProceedingBtnTapped), for: .touchUpInside)
    ...
    @objc func ProceedingBtnTapped() {
        print("ProceedingBtnTapped - called()")
        self.navigationController?.pushViewController(NewsWebViewController(post: post.bill_name), animated: true)
    }
